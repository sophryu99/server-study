...
🌟 **데이터베이스 정규화**란? : RDB 설계를 직관적이고 논리적으로 만드는 과정

- 불필요한 데이터 제거
- 데이터의 중복 최소화 데이터를 다루면서 생기는 이상현상 방지
...
1차 정규화: Row 마다 칼럼의 값이 1개씩만 있어야 함

2차 정규화: 부분 함수적 종속을 제거

- X의 값을 알면 Y의 값을 바로 식별할 수 있고, X의 값에 Y의 값이 달라질 때, Y는 X에 함수적 종속이라고 함

3차 정규화: 이행적 함수 종속을 제거 

BCNF: 결정자가 후보 키가 아니면 제거
